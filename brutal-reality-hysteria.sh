...
show_client_configuration() {
  ...
  reality_brutal_port=$(jq -r '.inbounds[] | select(.tag == "vless-brutal-in") | .listen_port' /root/sbox/sbconfig_server.json)
...
echo ""
echo "输入vision reality端口号"
reality_port=$(generate_port)
echo "输入brutal reality端口号"
reality_brutal_port=$(generate_port)
info "生成的vision reality端口号为: $reality_port"
